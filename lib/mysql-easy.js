...
const ALLOWED_ISOLATION = new Set([
    'REPEATABLE READ',
    'READ COMMITTED',
    'READ UNCOMMITTED',
    'SERIALIZABLE',
...
class MySQLEasy {
    ...
    createTransaction(isolationLevel) {
        ...
        if (isolationLevel) {
            _isoLevel = isolationLevel.toUpperCase();

            if (!ALLOWED_ISOLATION.has(_isoLevel)) {
        ...
        return new Promise((resolve, reject) => {
            this._conn.getConnection((err, conn) => {
                ...
                if (err) {
                    ...
                    if (_isoLevel) {
                        conn.query('SET TRANSACTION ISOLATION LEVEL ' + _isoLevel, err => {
                            if (err) {
                                conn.destroy();
                                reject(err);
                            } else {
                    ...
                    function next() {
                        conn.beginTransaction(err => {
                            if (err) {
                                ...
                                transaction._profiling = that._profiling;
