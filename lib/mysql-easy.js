...
class MySQLEasy {
    ...
    [_select](tableName, fields, join, where, order, limit, offset) {
        if (!tableName) {
...
function getJoin(join) {
    ...
    joinParts.push('JOIN', mysql.escapeId(join.table), 'ON', getWhere(join.on));
        ...
        if (type === 'string') {
            ...
            for (var fieldName in where) {
                if (where.hasOwnProperty(fieldName)) {
                    var escapedField = mysql.escapeId(fieldName);
...
function formatValue(data) {
    if (data != null) {
        if (data.$raw) {
            ...
            return mysql.escapeId(data.$field);
        }
    }
