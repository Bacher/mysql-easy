...
class MySQLEasy {
    ...
    query(sqlQuery, params) {
        if (this._profiling) {
            ...
            return new Promise((resolve, reject) => {
                this._conn.query(sqlQuery, params, function(err, res) {
                    ...
                    if (err) reject(err);
                    else resolve(this._processResults(res));
                });
            });
        } else {
            return new Promise((resolve, reject) => {
    ...
    /**
     * Создать экземпляр транзакции (доступно только для типа соединения Pool).
     *
